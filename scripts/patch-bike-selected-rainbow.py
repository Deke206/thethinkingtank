...
# Remove the Rainbow-only hardcoded recipe override so selected colors remain authoritative.
s, count = re.subn(
...
# Keep the normal three-color, gradient-length, speed, and direction controls visible for Rainbow.
...
# Bike starts visually off every time; saved presets are still loaded from the shared preset key.
...
# Current compact buttons are 112x86. Make them slightly smaller without changing the two-column mobile behavior.
